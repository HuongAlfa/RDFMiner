...
public class SparqlEndpoint implements Iterator<QuerySolution>
{
	...
	{	File file = new File("TDt");
		if(file.exists())
		{
			logger.warn("A local TDB directory exists. Using it as the SPARQL endpoint");
			endpoint = null;
			Dataset dataset = TDBFactory.createDataset("TDst");
					//media/huong/WORKING/These/Codes/Java/rdfminer/tdb");
			tdb = dataset.getDefaultModel();
		}
		else
		{
			logger.warn("A local TDB directory does not exists. Using the default SPARQL endpoint " + url);
			endpoint = url;
