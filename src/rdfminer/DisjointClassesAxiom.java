...
public class DisjointClassesAxiom extends Axiom
{
	...
	{
		disjointClass = new Expression[arguments.size()];// chứa các argument Arg của Axiom. ex: DisjointClass(Arg1, Arg2)
		disjointClassComplement = new Expression[disjointClass.length];
		for(int i = 0; i<disjointClass.length; i++)
		{
			disjointClass[i] = ExpressionFactory.createClass(arguments.get(i)); // chứa Arg của Axiom dưới dạng Expression	
	...
	protected String disjunctionGraphPattern2(String x, String y)
	{
	//	String dc = Expression.getFreshVariableName();
		String z1 = Expression.getFreshVariableName();
	//	String z2 = Expression.getFreshVariableName();
	...
	public void update()
	{
		...
		referenceCardinality = RDFMiner2.endpoint.count("?x", refCardGraphPattern);
		...
		numConfirmations = RDFMiner2.endpoint.count("?x", confirmationGraphPattern);
		...
		numExceptions = RDFMiner2.endpoint.count("?x", exceptionGraphPattern);
	...
	public void update2()   // added by NGUYEN THU HUONG
	{
		confirmations = new ArrayList<String>();
		exceptions = new ArrayList<String>();
		...
		for(int i = 0; i<disjointClass.length; i++)
		{
			...
			for(int j = 0; j<disjointClass.length; j++)
			{
			    ...
			    //confirmationGraphPattern += "FILTER";
				//	confirmationGraphPattern += disjunctionGraphPattern(j, i, "?x", "?y");		
				//	confirmationGraphPattern += disjointClass[i].graphPattern + "\n ";
				    confirmationGraphPattern +=disjointClassComplement[j].graphPattern + "\n";
			}
			confirmationGraphPattern += " }";
		}
		confirmationGraphPattern += " } ";
		...
		if(numConfirmations>0 && numConfirmations<100)
		{
			// query the confirmations
		//	RDFMiner.endpoint.select("TO DO");
		//	RDFMiner2.endpoint.select(confirmationGraphPattern);
			while(RDFMiner2.endpoint.hasNext())
			{
		    	QuerySolution solution = RDFMiner2.endpoint.next();
	    		RDFNode x = solution.get("x");
	...
	public void update3()   // added by NGUYEN THU HUONG
	{
		confirmations = new ArrayList<String>();
		exceptions = new ArrayList<String>();
	...
	/*	String confirmationGraphPattern = "";
		...
		*/
	    ...
	    logger.info("number of Confirmations: " + numConfirmations);
	}
