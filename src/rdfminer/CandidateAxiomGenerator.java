...
public class CandidateAxiomGenerator extends AxiomGenerator
{
	...
	{
		super(fileName); // call AxiomGenerator(fileName) in superclass
		...
		try
		{
			...
			int subClassIndex = rule.size();  //the number of productions in the rule for "Class" 
		...
		catch(IOException e)// khong ton tai AxiomGenerator.status thi thuc hien thong bao
		{
	 ...
	 */
	protected Set<RDFNodePair> getNodes(String sparql)  // thu duoc gia tri cua Node sau khi thuc hien sparql
	{
	...
	//public GEIndividual AxiomIndividual()
	 //{
